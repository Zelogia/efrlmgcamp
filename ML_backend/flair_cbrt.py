...
#         flair_trainer_path_seq = os.path.join(training_path, "flair_ner",timestamp_folder)
        ...
        model_path=os.path.join(flair_trainer_path_seq,"best-model.pt") #best-model.pt appears at end of training, if bug use final-model.pt
        ml_flair_seq = flair.models.SequenceTagger.load(model_path)
        
        model_name = "flare_seq"
    ...
    def fit(self, annotations, working_dir=None, **kwargs):
        ...
        flair_trainer_path_seq = os.path.join(training_path, "flair_ner")
        ...
        print(len(corpus.train),len(corpus.dev))
        ...
        embeddings_cbrt_base = TransformerWordEmbeddings('camembert-base',
        ...
        tagger_cbrt_base = initialize_tagger(embeddings=embeddings_cbrt_base, tag_type=tag_type,
                        tag_dictionary=tag_dictionary, cbrt=True)
        # 6. train
        trainer : ModelTrainer = ModelTrainer(tagger_cbrt_base, corpus)
        trainer.train(flair_trainer_path_seq,
                        use_tensorboard=True,
                        tensorboard_log_dir=Tensorboard_dir,
                        optimizer=AdamW,#TOEDIT
                        learning_rate=5e-5,#TOEDIT
                        min_learning_rate=0.000001,#TOEDIT
                        mini_batch_size=32,#TOEDIT
                        patience=10,#TOEDIT
                        max_epochs=7,#TOEDIT
                        num_workers=20) #TOEDIT
